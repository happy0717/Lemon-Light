...
- 底部横幅：背景色、不透明度、布局/行数/字号/字重/字体/滚动速度、交易时段自动显隐、自定义位置记忆；

- 悬浮球：展开面板背景色与不透明度、休市自动隐藏（含午休与周末，开市自动恢复）；
...
- `dist/Lemon Light Setup 1.0.0.exe` —— **NSIS 安装版**：可自定义安装目录、创建桌面快捷方式；

- `dist/LemonLight-1.0.0-portable.exe` —— **单文件便携版（无需安装）**：双击直接运行，
