...
- **个股显示模式可切换**（设置页「外观与行为 → 悬浮球 → 个股显示」）：

  - **当日涨幅**：圆球轮播展示当前自选的现价与当日涨跌幅（默认）；
  - **持仓收益**：折叠圆球直接展示**全部持仓的总盈亏**（顶部带"持仓收益"标签，金额在上、收益率在下，
    金额 ≥1 万自动以「万」缩写，颜色随盈亏方向）；展开面板列表对有持仓的自选逐只显示收益金额与收益率，
    未录持仓的自选自动回退显示当日涨幅，避免误读。
...
- **股票后展示持仓盈亏（默认关闭）**：在设置页开启后，录入过持仓的股票会在当日涨跌之后追加显示
  `盈亏金额 | 收益率`（按最新价与含佣金税费的持仓成本实时估算，颜色随盈亏方向），当日涨幅与累计持仓盈亏分行展示、不易混淆。

> 界面预览：开启「股票后持仓盈亏」后的横幅行情条目效果。

<img src="./img/横幅展示盈亏信息.png" alt="底部横幅展示持仓盈亏信息" width="840" />
...
> 界面预览：设置页「持仓盈亏」——录入买卖流水（字段均带单位）、费用规则配置与当前持仓 / 交易记录表格。

<img src="./img/持仓盈亏配置.png" alt="持仓盈亏 · 交易录入与费用规则" width="840" />

- 录入 **买入 / 卖出** 流水（股票、方向、成交价、股数、成交日期），"当前持仓"与"交易记录"均带**表头与单位**
  （成本（元）/ 市值（元）/ 盈亏（元）/ 收益率 / 佣金 / 税费），字段含义一目了然；

- **佣金、税费自动填充，且可手动修改**：

  - **佣金**按市场分别配置"费率（万分之 X）+ 单笔最低佣金（0 = 不设最低）"，自动按
    `max(成交额 × 费率, 最低佣金)` 计算；填入错误可一键「恢复自动」重新按规则填充；

  - **税费**按市场法定税率自动计算：A股卖出印花税 0.05%、港股买卖各 0.1%、美股无；
    实际不符时可手动修改。

- **成本 = 成交额 + 佣金 + 税费**，佣金与税费计入持仓成本与盈亏统计，汇总区实时显示累计费用合计；

- 当日 / 持仓盈亏汇总可直接在悬浮球面板查看。

### 外观与行为（Appearance）

> 界面预览：设置页「外观与行为」——横幅自定义（布局 / 行数 / 字号 / 颜色 / 透明度）、老板键配置与盈亏信息展示开关。

<img src="./img/事实盯盘支持横幅自定义设置.png" alt="横幅自定义设置" width="840" />

<img src="./img/支持老板键一键关闭横幅.png" alt="老板键一键关闭横幅" width="840" />

<img src="./img/横幅和悬浮球支持配置是否展示盈亏信息.png" alt="横幅 / 悬浮球盈亏信息展示开关" width="840" />

- 底部横幅：背景色、不透明度、布局/行数/字号/字重/字体/滚动速度、交易时段自动显隐（每日开盘前 5 分钟提前显示、收盘后 30 分钟内保持显示尾盘价）、自定义位置记忆；

- **盈亏信息展示开关**：横幅的「股票后持仓盈亏」（默认关闭）与悬浮球的「个股显示」模式（当日涨幅 / 持仓收益）均在此页即时生效并全局同步（悬浮球/横幅/设置页多窗口同屏联动）；
...
- `dist/Lemon Light Setup 1.0.2.exe` —— **NSIS 安装版**：可自定义安装目录、创建桌面快捷方式；

- `dist/LemonLight-1.0.2-portable.exe` —— **单文件便携版（无需安装）**：双击直接运行，
